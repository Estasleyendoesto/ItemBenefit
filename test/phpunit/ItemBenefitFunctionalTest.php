...
/**
 * \file    test/phpunit/ItemBenefitFunctionalTest.php
 * \ingroup itembenefit
...
/**
 * Class ItemBenefitFunctionalTest
 *
 ...
 *
 * @package Testitembenefit
 */
class ItemBenefitFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modItemBenefit')->click();
		} else {
			// Disable the module
			$this->byHref('modItemBenefit')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/itembenefit/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('itembenefit/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/itembenefit/admin/about.php');
		$this->authenticate();
		return $this->assertContains('itembenefit/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('itembenefitwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modItemBenefit_ItemBenefitTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modItemBenefit_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
